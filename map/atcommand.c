...
ACMD_FUNC(iteminfo)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (item_data->nameid < 26000) // Custom Items not tracked on @ii by Mano de Dios
		{
		...
		clif_displaymessage(fd, atcmd_output);
			...
			sprintf(atcmd_output, "The requested item cannot be tracked.");
			clif_displaymessage(fd, atcmd_output);
		}
...
ACMD_FUNC(whodrops)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (item_data->mob[0].chance == 0) {
			...
			sprintf(atcmd_output, "The item cannot be tracked.");
			clif_displaymessage(fd, atcmd_output);
		}
	}
	return 0;
